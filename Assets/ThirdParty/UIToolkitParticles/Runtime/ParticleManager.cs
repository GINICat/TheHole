    ...
    public class ParticleManager
    {
        ...
        public void Update()
        {
            ...
            foreach (var image in _images)
            {
                if (isActive(image))
                {
                    image.ParticleSystemForImage?.ResumeDueToActivity();
                    image.MarkDirtyRepaint();
                }
                else
                {
        ...
        private static bool isActive(ParticleImage image)
        {
            return image.visible && image.enabledInHierarchy && image.resolvedStyle.display != DisplayStyle.None && image.resolvedStyle.opacity > 0f;
        }
