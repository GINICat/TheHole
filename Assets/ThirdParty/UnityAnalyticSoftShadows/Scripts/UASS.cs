...
public class UASS : MonoBehaviour
{
    ...
    void Reset()
    {
        if (GraphicsSettings.renderPipelineAsset == null) Debug.LogWarning("[UASS] This version of Analytic Soft Shadows requires the Universal render pipeline");
        OnValidate();
    }
    void OnValidate()
    {
    ...
    int CamWidth(Camera cam) => (int)(cam.pixelWidth * rendScale);
    int CamHeight(Camera cam) => (int)(cam.pixelHeight * rendScale);
    void OnResolutionOrSettingsChange()
    {
        if (!light || !light.isActiveAndEnabled) return;

        float camWidth = CamWidth(cam);
        float camHeight = CamHeight(cam);

        int numFrustumsX = Mathf.CeilToInt(camWidth / (float)tileSize);
        int numFrustumsY = Mathf.CeilToInt(camHeight / (float)tileSize);
        int numTotalFrustrums = numFrustumsX * numFrustumsY;

        //print(System.Runtime.InteropServices.Marshal.SizeOf(typeof(LightData)));
        lightListBuffer = new ComputeBuffer(maxLights, UnsafeUtility.SizeOf<LightData>());
        ...
        buf.SetComputeVectorParam(lightCullingCompute, "data", new Vector4(1.0f / camWidth, 1.0f / camHeight, numFrustumsX, numFrustumsY));
        ...
        buf.GetTemporaryRT(lowRes, Mathf.CeilToInt(camWidth / (float)(downscale + 1)), Mathf.CeilToInt(camHeight / (float)(downscale + 1)), 0, FilterMode.Bilinear);
        ...
        //Debug.Log(new { cam.name, camWidth, camHeight, rendScale });
    }
    void Cleanup()
    {
    ...
    uint[] zeroArray = new uint[1] { 0 };

    public void OnPreRenderScriptablePipeline(float renderScale)
    {
        ...
        rendScale = renderScale;
        if (light && light.type == LightType.Point) algorithm = Algorithm.Indirect;


        float Switch(float a, float b, float c, int i)
        {
            if (i == 0) return a;
            if (i == 1) return b;
            if (i == 2) return c;
        ...
        downscale = (int)quality < 3 ? Mathf.Max(1, Mathf.RoundToInt(Mathf.Lerp(Switch(0f, 0f, 1f, (int)quality), algorithm == Algorithm.Direct ? Switch(3f, 6f, 12f, (int)quality) : Switch(2f, 4f, 8f, (int)quality), lightAngle / 180f))) : (int)quality - 3;
        int camWidth = CamWidth(cam);
        int camHeight = CamHeight(cam);
        if (lightsGridTex == null
            || camWidth != prevScreenWidth || camHeight != prevScreenHeight || downscale != prevDownscale || algorithm != prevAlgorithm
            || !light || light.isActiveAndEnabled != prevLightActive)
        {
            prevScreenWidth = camWidth;
            prevScreenHeight = camHeight;
        ...
        currentLightIndexBuffer.SetData(zeroArray);
        ...
        material.SetVector("_CurrentTexelSize", new Vector4(1f / camWidth, 1f / camHeight, camWidth, camHeight));
        ...
        foreach (UASSSphere sphereObj in Spheres/*.Where(x => x)*//*.OrderBy(x => Vector3.Distance(cam.transform.position, x.transform.TransformPoint(x.center)))*/)
        {
            ...
            if (noCpuCulling || CappedConeInsideFrustum(boundingConePos, lightDir, boundingConeLength, boundingConeRadius, boundingConeBackadd, frustumPlanes, furthestPointDirections))
            {
                lights[index++] = new LightData()
                {
                    ...
                    color = new Vector3(sphereObj.color.r, sphereObj.color.g, sphereObj.color.b)
                };
        ...
        foreach (UASSCapsule capsuleObj in Capsules/*.Where(x => x)*//*.OrderBy(x => Vector3.Distance(cam.transform.position, x.transform.TransformPoint(x.center)))*/)
        {
            ...
            float backadd = k * max;
            Vector3 boundingConePos = capsuleObj.cachedTransform.TransformPoint(capsuleObj.center) - lightDir * backadd;
            float boundingConeLength = algorithm == Algorithm.Indirect ? (k * /*5.775f*/2.8875f * max + backadd + max) : 1000f;
            ...
            if (noCpuCulling || CappedConeInsideFrustum(boundingConePos, lightDir, boundingConeLength, boundingConeRadius * coneRadiusMul, boundingConeBackadd, frustumPlanes, furthestPointDirections))
            {
                lights[index++] = new LightData()
                {
                    ...
                    color = new Vector3(capsuleObj.color.r, capsuleObj.color.g, capsuleObj.color.b)
                };
                if (index == maxLights) break;
            }
        }
            ...
            foreach (UASSBox boxObj in Boxes/*.Where(x => x)*//*.OrderBy(x => Vector3.Distance(cam.transform.position, x.transform.TransformPoint(x.center)))*/)
            {
                ...
                if (noCpuCulling || CappedConeInsideFrustum(boundingConePos, lightDir, boundingConeLength, boundingConeRadius, boundingConeBackadd, frustumPlanes, furthestPointDirections))
                {
                    ...
                    lights[index++] = new LightData()
                    {
                        ...
                        color = new Vector3(boxObj.color.r, boxObj.color.g, boxObj.color.b)
                    };
                    if (index == maxLights) break;
                }
            }
    ...
    {
        var m = capsule.cachedTransform.localToWorldMatrix;
        
        center = m.MultiplyPoint3x4(capsule.center);
        radius = 0f;
        height = 0f;
        Vector3 lossyScale = m.lossyScale;
        ...
        switch ((int)capsule.direction)
        {
            case 0: // x
                ...
                dir = m.MultiplyVector(Vector3.right);
                break;
            case 1: // y
                ...
                dir = m.MultiplyVector(Vector3.up);
                break;
            case 2: // z
                ...
                dir = m.MultiplyVector(Vector3.forward);
                break;
        }
    ...
    bool CappedConeInsideFrustum(Vector3 origin, Vector3 forward, float size, float angle, float backadd, Vector4[] frustumPlanes, Vector3[] furthestPointDirectionsTimesAngle)
    {
        for (int i = 0; i < 6; i++)
        {
            Vector4 a = frustumPlanes[i];

            // The compiler is able to optimize this mess down a heck of a lot more than UnityEngine.Vector3 struct operations. This function goes from 0.6 ms to 0.01 ms. Massive cpu perf win.
            
            Vector3 f = furthestPointDirectionsTimesAngle[i];
            
            _Vector3 b3 = new _Vector3();
            b3.x = forward.x * backadd;
            b3.y = forward.y * backadd;
            b3.z = forward.z * backadd;
            _Vector3 a3 = new _Vector3();
            a3.x = origin.x + b3.x;
            a3.y = origin.y + b3.y;
            a3.z = origin.z + b3.z;
            _Vector3 b1 = new _Vector3();
            b1.x = f.x * backadd;
            b1.y = f.y * backadd;
            b1.z = f.z * backadd;
            _Vector3 b = new _Vector3();
            b.x = a3.x - b1.x;
            b.y = a3.y - b1.y;
            b.z = a3.z - b1.z;
            
            _Vector3 b4 = new _Vector3();
            b4.x = forward.x * size;
            b4.y = forward.y * size;
            b4.z = forward.z * size;
            _Vector3 a4 = new _Vector3();
            a4.x = origin.x + b4.x;
            a4.y = origin.y + b4.y;
            a4.z = origin.z + b4.z;
            _Vector3 b2 = new _Vector3();
            b2.x = f.x * size;
            b2.y = f.y * size;
            b2.z = f.z * size;
            _Vector3 c = new _Vector3();
            c.x = a4.x - b2.x;
            c.y = a4.y - b2.y;
            c.z = a4.z - b2.z;

            if (a.x * b.x + a.y * b.y + a.z * b.z + a.w * 1.0f < 0.0f)
            {
                if (a.x * c.x + a.y * c.y + a.z * c.z + a.w * 1.0f < 0.0f)
                {
                    return false;
                }
            }
