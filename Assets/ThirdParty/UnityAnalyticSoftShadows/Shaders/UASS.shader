	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				uint2 grid = lightsGrid[(uint2)(float2(uv.x, 1.0 - uv.y)/*<-*/ * _CurrentTexelSize.zw / 32.0)];
				uint n = min(1024, grid.y);
				if (n == 0) return 1.0;

				// read depth and reconstruct world position
				float2 texelSize = 0.5 * _CurrentTexelSize.xy;
				...
				for (uint i = 0; i < n; ++i)
				{
				    ...
				    Light light = lights[lightsIndexBuffer[grid.x + i]];
					float4x4 mat = light.mat;
					float3 minMaxDist = light.minMaxDist;
					float3 biasedWsPos = wsPos + lightDir * max(bias, minMaxDist.x);
					if (mat[3][3] == 1) //SPHERE
					{
					    #ifdef INDIRECT_ALGO
					    	float3 pos = float3(mat[0][0], mat[1][0], mat[2][0]) +lightDir*mat[3][0]/4;
					    	shadow *= lerp(light.color, 1.0, sphereShadow(biasedWsPos, lightDir, lightAngle, float4(pos.x, pos.y, pos.z, mat[3][0]), minMaxDist.y, lightVectorLength));
					    	//shadow *= sphereShadow(wsPos, lightDir, lightAngle, float4(mat[0][0], mat[1][0], mat[2][0], mat[3][0]), minMaxDist.y);
					    #else
					    	shadow *= lerp(light.color, 1.0, sphereShadow(biasedWsPos, lightDir, float4(mat[0][0], mat[1][0], mat[2][0], mat[3][0]), k * 0.5));
					    #endif
					}
					else if (mat[3][3] == 2) //CAPSULE
					{
					    	...
					    	shadow *= lerp(light.color, 1.0, capsuleShadow(/*wsPos*/biasedWsPos, lightDir, lightAngle, CapsuleCenterAndRadius, CapsuleOrientationAndLength, minMaxDist.y, lightVectorLength));
					    #else
					    	shadow *= lerp(light.color, 1.0, capsuleShadow(biasedWsPos, lightDir, float3(mat[0][0], mat[1][0], mat[2][0]), float3(mat[0][1], mat[1][1], mat[2][1]), mat[3][0], k));
					    #endif
					}
					else if (mat[3][3] == 3 || mat[3][3] == 4) //BOX
					{
						...
						shadow *= lerp(light.color, 1.0, boxShadow(biasedWsPos, lightDir, mat, v.xyz, k, v.w == 4, minMaxDist.y));
					}
					//else if (5) //MASK_SPHERE //if pixel inside sphere/Capsule/Box shadow=1.0 break;
					if (any(shadow <= 0.0)) break;
				}
		...
		Pass
		{
			...
			void frag(v2f i, out float4 outDiffuse : SV_Target0)
			{
				...
				float4 TextureValues00 = tex2D(_SourceTex, Corner00UV);//xyz is shadow, w is depth used
				float4 TextureValues10 = tex2D(_SourceTex, Corner00UV + float2(LowResTexelSize.x, 0));
				float4 TextureValues01 = tex2D(_SourceTex, Corner00UV + float2(0, LowResTexelSize.y));
				float4 TextureValues11 = tex2D(_SourceTex, Corner00UV + LowResTexelSize);
				...
				float3 shadow = InterpolatedResult;
				//
				
				if (any(isnan(shadow))) shadow = 1.0;

				outDiffuse = float4(shadow, 1.0);
			}
